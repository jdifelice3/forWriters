...
const createMemberConnectRequest = async (authId, collaboratorId) => {
    ...
    if (!user) {
        const joinGroupError = new Error_1.JoinRequestError("User not found.", 404);
    ...
    if (existingConnection.length > 0) {
        const joinGroupError = new Error_1.JoinRequestError("You have already connected to this member.", 400);
        throw joinGroupError;
    }
    ...
    if (existingRequest) {
        const joinGroupError = new Error_1.JoinRequestError("You already have a pending request with this member.", 400);
        throw joinGroupError;
    }
...
const approveConnectRequest = async (connectRequestId, authId) => {
    ...
    if (connectReq.status !== client_1.JoinRequestStatus.PENDING) {
...
const rejectConnectRequest = async (connectRequestId, authId) => {
    ...
    if (!connectReq) {
        throw new Error_1.JoinRequestError("Connect request not found.", 404);
    }
    if (connectReq.status !== client_1.JoinRequestStatus.PENDING) {
        throw new Error_1.JoinRequestError("This request is no longer pending.", 400);
    }
