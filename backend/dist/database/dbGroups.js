...
const getGroupByUserId = async (userId) => {
    try {
        ...
        if (!groupsByUserId) {
            throw new domain_types_1.GroupError('User has not joined any groups', 404);
        }
...
const createJoinGroupRequest = async (authId, groupId) => {
    ...
    if (!group) {
        const joinGroupError = new domain_types_1.JoinRequestError("Group not found.", 404);
    ...
    if (existingMember) {
        const joinGroupError = new domain_types_1.JoinRequestError("You are already a member of this group.", 400);
        throw joinGroupError;
    }
    ...
    if (existingRequest) {
        const joinGroupError = new domain_types_1.JoinRequestError("You already have a pending join request.", 400);
        throw joinGroupError;
    }
...
const approveJoinRequest = async (joinRequestId, authId) => {
    ...
    if (joinReq.status !== domain_types_1.JoinRequestStatus.PENDING) {
    ...
    if (!adminMembership || !adminMembership.isAdmin) {
        throw new domain_types_1.JoinRequestError("You are not an admin for this group.", 403);
    }
