...
const createReading = async (groupId, name, createdUserId, readingDate, readingStartTime, readingEndTime, submissionDeadline, description, schedule) => {
    try {
        ...
        console.log('scheduledType  === ReadingSchedule.SCHEDULED', scheduledType === domain_types_1.ReadingScheduleType.SCHEDULED);
        const reading = await prisma.reading.create({
            data: {
                groupId: groupId,
                readingDate: scheduledType === domain_types_1.ReadingScheduleType.SCHEDULED ? new Date(readingDate) : null,
                submissionDeadline: scheduledType === domain_types_1.ReadingScheduleType.SCHEDULED ? new Date(submissionDeadline) : null,
                name: name,
                createdUserId: createdUserId,
                readingStartTime: scheduledType === domain_types_1.ReadingScheduleType.SCHEDULED ? readingStartTime : null,
                readingEndTime: scheduledType === domain_types_1.ReadingScheduleType.SCHEDULED ? readingEndTime : null,
...
const deleteReading = async (readingId, groupId) => {
    ...
    if (!readingInGroup) {
        throw new domain_types_1.ReadingDeleteInvalidGroupIdError("You are trying to delete a reading that does not belong to your group.", 403);
    }
    ...
    if (authors.length > 0) {
        throw new domain_types_1.ReadingDeleteError("You cannot delete a reading that has authors associated with it.", 403);
    }
    ...
    console.log(JSON.stringify(exports.deleteReading));
